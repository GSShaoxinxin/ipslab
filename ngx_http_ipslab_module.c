...
//浏览器使用的ID加密算法，根据收到的ID生成请求携带的ID
#define hx_ID_Encrypt_Algorithm MD5
//服务器使用的ID陆续生成算法，由slab中当前ID生成下一代ID。
...
//请在副本函数之前ctx = duk_create_heap_default();之后duk_destroy_heap(ctx);
	//成功返回1，失败返回0
	//在本函数外执行后使用duk_get得到返回结果
	if (!ctx) {
		return 1;
	}
	...
	if(duk_pcompile(ctx, DUK_COMPILE_FUNCTION)==0){
		...
		duk_push_lstring(ctx,hx_cont,hx_cont_len);//压进参数，即要混淆的内容
